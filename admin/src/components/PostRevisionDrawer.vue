...
import { getRevisions, getRevision, restoreRevision } from '@/api/posts'
import { countLines, diffLines, DIFF_RENDER_LINE_LIMIT } from '@/utils/diff'
import { formatTime } from '@/utils/format'
import type { AdminPostItem, PostRevisionItem } from '@/types/api'

/**
 * 文章版本历史抽屉：版本列表 / 行级对比（统一视图 Diff）/ 恢复
 * 恢复由服务端先把当前内容快照为新版本（「恢复前快照」），再应用目标版本，可再次恢复撤销。
 */
...
const emit = defineEmits<{
  ...
  (e: 'restored', post: AdminPostItem): void
}>()
...
// 版本列表
// ---------------------------------------------------------------------------
const list = ref<PostRevisionItem[]>([])
const total = ref(0)
const page = ref(1)
...
async function loadList() {
  ...
  try {
    const result = await getRevisions(props.postId, { page: page.value, pageSize: PAGE_SIZE })
...
async function openCompare(version: number) {
  const postIdValue = props.postId
  if (postIdValue === null) return
  ...
  try {
    const result = await getRevision(postIdValue, version)
...
async function onTargetChange() {
  const postIdValue = props.postId
  if (postIdValue === null || baseVersion.value === null) return
  ...
  compareLoading.value = true
  try {
    const result = await getRevision(postIdValue, compareTarget.value)
    if (seq !== compareSeq) return
...
function onRestore(item: PostRevisionItem) {
  const postIdValue = props.postId
  if (postIdValue === null) return
  Modal.confirm({
    title: `恢复到 v${item.version}`,
    content: `将用 v${item.version} 的内容覆盖当前内容；当前内容会先自动保存为新版本，可再次恢复撤销。`,
    okText: '恢复',
    cancelText: '取消',
    onOk: async () => {
      if (props.postId === null) return
      ...
      try {
        const result = await restoreRevision(props.postId, item.version)
        message.success(`已恢复到 v${item.version}，编辑器内容已同步更新`)
        emit('restored', result.post)
    ...
    <template v-if="baseVersion === null">
      <a-spin :spinning="listLoading">
        ...
        <div v-else-if="list.length === 0" class="revision-drawer__empty">
          <a-empty description="暂无历史版本，保存文章后自动生成" />
        </div>
